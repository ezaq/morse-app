...
<body>
  ...
  <script>
    ...
    const darkDurations = [];
    // ドット・ダッシュのしきい値
    let dotDashThreshold = 150; // ms 初期仮設定
    ...
    histogramCanvas.addEventListener('click', (e) => {
      ...
      // alert(`ドット/ダッシュしきい値を ${dotDashThreshold} ms に設定しました`);
    });

    // ヒストグラムからデコードする関数
    function decodeFromDurations() {
      let morseSequence = '';
      let result = '';

      // lightDurationsとdarkDurationsを順番に処理する
      const minLength = Math.min(lightDurations.length, darkDurations.length);

      for (let i = 0; i < minLength; i++) {
        const light = lightDurations[i];
        const dark = darkDurations[i];

        if (light < dotDashThreshold) {
          morseSequence += '.'; // ドット
        } else {
          morseSequence += '-'; // ダッシュ
        }

        if (dark > WORD_SPACE * 0.7) {
          // 単語間スペース（単語区切り）
          if (morseSequence.length > 0) {
            result += REVERSE_MORSE[morseSequence] || '?';
            morseSequence = '';
          }
          result += ' '; // 単語の間に空白を追加
        } else if (dark > LETTER_SPACE * 0.7) {
          // 文字間スペース
          if (morseSequence.length > 0) {
            result += REVERSE_MORSE[morseSequence] || '?';
            morseSequence = '';
          }
          // 文字間スペースでは空白は入れない（単語の中なので）
        }
      }
    ...
    decodeBtn.style.fontSize = '2.4em';
    decodeBtn.onclick = decodeFromDurations;
    document.querySelector('.controls').appendChild(decodeBtn);
